...
class LobbyPage extends Page {
  ...
  render() {
    const playersHTML = this.props.players.length
      ? this.props.players.map(p => `<li>${p.name}${p.ready ? ' ✅' : ''}</li>`).join('')
      : `<li class="waiting">No players yet...</li>`;

    return html`
      <h1>Game Lobby</h1>
      <h3 class="lobby-id-container">
        Lobby ID: <span class="lobby-id">${this.props.lobbyId}</span>
      </h3>

      <h2>${this.props.players.length > 0 ? 'Waiting for Players...' : 'Preparing Lobby...'}</h2>

      <ol class="player-list">${playersHTML}</ol>

      <div class="button-group">
        <button class="ready-button">${this.props.ready ? 'Ready ✔' : 'Ready'}</button>
  ...
  mounted() {
    this.addShadowListener('click', (e) => {
      ...
      if (e.target.closest('.leave-button')) {
        this.dispatchEvent(new CustomEvent('lobby-leave', { bubbles: true, composed: true }));
        this.navigate('login');
      }
    });
