...
class LobbyPage extends Page {
  ...
  render({ props }) {
    const playersHTML = props.players.length > 0 ?
      props.players.map(p => `<li>${p.name}${p.ready ? ' ✅' : ''}</li>`).join('') :
    ...
    return html`
      ...
      <h2>${props.players.length > 0 ? 'Waiting for Players...' : 'Preparing Lobby...'}</h2>

      <ol class="player-list">${playersHTML}</ol>

      <div class="button-group">
        <button class="ready-button">${props.ready ? 'Ready ✔' : 'Ready'}</button>
